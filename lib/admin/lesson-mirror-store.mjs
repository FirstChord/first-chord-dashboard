...
/**
 * Returns a bounded, adapter-only detail set for lesson-shaped events that were
 * inside the latest verified window but were not re-observed in that run.
 * Provider aliases are included only so the server composer can resolve names;
 * they must never be returned by a page or assistant-facing projection.
 */
export async function getLessonMirrorExceptionDetails({
  limit = 250,
  database = null,
  env = process.env,
  now = new Date(),
  ...
  if (!Number.isInteger(limit) || limit < 1 || limit > 500) {
    throw new Error('Lesson-exception detail limit must be between 1 and 500');
  }
  ...
  if (!verified) return { source, totalCount: 0, details: [] };
  ...
  const result = await databaseClient.query(`
    WITH latest AS (
      ...
      CROSS JOIN latest
      WHERE event.last_observed_at < latest.started_at
        AND EXISTS (
          SELECT 1
          FROM fc_lesson_participations participation
          WHERE participation.fc_event_id = event.fc_event_id
        )
    ), current_events AS (
      SELECT event.*
      FROM window_events event
      ...
      WHERE event.last_observed_at >= latest.started_at
        AND event.calendar_observed = TRUE
    ), bounded_stale_lessons AS (
      SELECT stale.*, COUNT(*) OVER()::integer AS total_count
      FROM stale_lessons stale
      ORDER BY stale.local_date ASC, stale.local_time ASC, stale.fc_event_id ASC
      LIMIT $2::integer
    )
    SELECT
      stale.fc_event_id,
      stale.fc_series_id,
      stale.local_date::text AS local_date,
      TO_CHAR(stale.local_time, 'HH24:MI:SS') AS local_time,
      stale.duration_minutes,
      stale.tutor_external_id,
      stale.original_tutor_external_id,
      stale.location_name,
      stale.category_name,
      stale.source_status,
      stale.source_recurring,
      stale.first_observed_at,
      stale.last_observed_at,
      stale.total_count,
      (
        SELECT COUNT(*)::integer
        FROM current_events current_event
        WHERE stale.fc_series_id IS NOT NULL
          AND current_event.fc_series_id = stale.fc_series_id
      ) AS current_same_series_count,
      COALESCE((
        SELECT JSONB_AGG(
          JSONB_BUILD_OBJECT(
            'fcParticipationId', participation.fc_participation_id,
            'studentExternalId', participation.student_external_id,
            'rawAttendanceStatus', participation.raw_attendance_status,
            'lastObservedAt', participation.last_observed_at
          ) ORDER BY participation.fc_participation_id
        )
        FROM fc_lesson_participations participation
        WHERE participation.fc_event_id = stale.fc_event_id
      ), '[]'::jsonb) AS participations,
      COALESCE((
        SELECT JSONB_AGG(
          JSONB_BUILD_OBJECT(
            'fcEventId', candidate.fc_event_id,
            'fcSeriesId', candidate.fc_series_id,
            'localDate', candidate.local_date,
            'localTime', candidate.local_time,
            'durationMinutes', candidate.duration_minutes,
            'tutorExternalId', candidate.tutor_external_id,
            'categoryName', candidate.category_name,
            'daysOffset', candidate.days_offset,
            'sameSeries', candidate.same_series,
            'sameSlot', candidate.same_slot,
            'matchedStudentExternalIds', candidate.matched_student_external_ids
          ) ORDER BY ABS(candidate.days_offset) ASC, candidate.local_date ASC, candidate.local_time ASC
        )
        FROM (
          SELECT
            current_event.fc_event_id,
            current_event.fc_series_id,
            current_event.local_date::text AS local_date,
            TO_CHAR(current_event.local_time, 'HH24:MI:SS') AS local_time,
            current_event.duration_minutes,
            current_event.tutor_external_id,
            current_event.category_name,
            (current_event.local_date - stale.local_date)::integer AS days_offset,
            (current_event.fc_series_id IS NOT NULL AND current_event.fc_series_id = stale.fc_series_id) AS same_series,
            (
              current_event.local_date = stale.local_date
            ...
            JSONB_AGG(DISTINCT current_participation.student_external_id) AS matched_student_external_ids
          ...
          WHERE stale_participation.fc_event_id = stale.fc_event_id
            ...
            AND current_event.local_date BETWEEN stale.local_date - 7 AND stale.local_date + 7
          GROUP BY
            current_event.fc_event_id,
            current_event.fc_series_id,
            current_event.local_date,
            current_event.local_time,
            current_event.duration_minutes,
            current_event.tutor_external_id,
            current_event.category_name
          ORDER BY
            ABS(current_event.local_date - stale.local_date) ASC,
            current_event.local_date ASC,
            current_event.local_time ASC
          LIMIT 8
        ) candidate
      ), '[]'::jsonb) AS nearby_candidates
    FROM bounded_stale_lessons stale
    ORDER BY stale.local_date ASC, stale.local_time ASC, stale.fc_event_id ASC
  `, [latestSuccessful.sync_run_id, limit]);
  const rows = result.rows || [];
  ...
  return {
    ...
    details: rows.map((row) => ({
      ...
      categoryName: clean(row.category_name),
      sourceStatus: clean(row.source_status),
      sourceRecurring: row.source_recurring === true,
      firstObservedAt: iso(row.first_observed_at),
      lastObservedAt: iso(row.last_observed_at),
      currentSameSeriesCount: Number(row.current_same_series_count) || 0,
      participations: Array.isArray(row.participations) ? row.participations.map((participation) => ({
        fcParticipationId: clean(participation.fcParticipationId),
        studentExternalId: clean(participation.studentExternalId),
        rawAttendanceStatus: clean(participation.rawAttendanceStatus),
        lastObservedAt: iso(participation.lastObservedAt),
      })) : [],
      nearbyCandidates: Array.isArray(row.nearby_candidates) ? row.nearby_candidates.map((candidate) => ({
        fcEventId: clean(candidate.fcEventId),
        fcSeriesId: clean(candidate.fcSeriesId),
        localDate: clean(candidate.localDate),
        localTime: clean(candidate.localTime),
        durationMinutes: candidate.durationMinutes === null || candidate.durationMinutes === undefined
          ? null
          : Number(candidate.durationMinutes),
        tutorExternalId: clean(candidate.tutorExternalId),
        categoryName: clean(candidate.categoryName),
        daysOffset: Number(candidate.daysOffset) || 0,
        sameSeries: candidate.sameSeries === true,
        sameSlot: candidate.sameSlot === true,
        matchedStudentExternalIds: Array.isArray(candidate.matchedStudentExternalIds)
          ? candidate.matchedStudentExternalIds.map(clean).filter(Boolean)
          : [],
      })) : [],
    })),
  };
