...
// Split_Billing rows -> Map<mmsId, [{customerId, subscriptionId, payerLabel}]>.
// Inactive rows are dropped here rather than filtered at each call site, so a
// household that stops splitting is forgotten everywhere at once.
export function buildSplitBillingIndex(rows = []) {
  const index = new Map();
  for (const row of rows) {
    const mmsId = `${row?.mms_id ?? row?.mmsId ?? ''}`.trim();
    if (!mmsId) continue;
    const active = `${row?.active ?? 'yes'}`.trim().toLowerCase();
    if (active === 'no' || active === 'false') continue;
    const customerId = `${row?.stripe_customer_id ?? row?.stripeCustomerId ?? ''}`.trim();
    const subscriptionId = `${row?.stripe_subscription_id ?? row?.stripeSubscriptionId ?? ''}`.trim();
    if (!customerId && !subscriptionId) continue;
    const payers = index.get(mmsId) || [];
    payers.push({
      customerId,
      subscriptionId,
      payerLabel: `${row?.payer_label ?? row?.payerLabel ?? ''}`.trim(),
    });
    index.set(mmsId, payers);
  }
  return index;
}

// The student's primary payer (from Students) followed by any Split_Billing
// payers, deduplicated so a row that repeats the primary cannot double-count.
function listStudentPayers(student = {}, splitBilling = new Map()) {
  const payers = [{
    customerId: `${student.stripeCustomerId || ''}`.trim(),
    subscriptionId: `${student.stripeSubscriptionId || ''}`.trim(),
    payerLabel: '',
  }];
  for (const extra of splitBilling.get(`${student.mmsId || ''}`.trim()) || []) {
    const duplicate = payers.some((payer) => (
      (extra.subscriptionId && payer.subscriptionId === extra.subscriptionId)
      || (!extra.subscriptionId && extra.customerId && payer.customerId === extra.customerId)
    ));
    if (!duplicate) payers.push(extra);
  }
  return payers.filter((payer) => payer.customerId || payer.subscriptionId);
}
...
export function buildStripeAmountsCacheRows(subscriptions = [], students = [], { now = new Date(), splitBillingRows = [] } = {}) {
  const splitBilling = buildSplitBillingIndex(splitBillingRows);
  ...
  for (const student of students) {
    ...
    // One row per payer. A split household has two real subscriptions and gets
    // two rows; buildStripeAmountsMap sums them back into one weekly figure.
    const matchedPayers = [];
    for (const payer of listStudentPayers(student, splitBilling)) {
      const mapped = (payer.subscriptionId && bySubscriptionId.get(payer.subscriptionId))
        || (payer.customerId && byCustomerId.get(payer.customerId))
        || null;
      if (!mapped) continue;
      if (matchedPayers.some((entry) => entry.mapped.subscriptionId === mapped.subscriptionId)) continue;
      matchedPayers.push({ payer, mapped });
    }
    if (!matchedPayers.length) {
      if (student.paymentMode === 'stripe') unmatchedStudents += 1;
      continue;
    }
    for (const { payer, mapped } of matchedPayers) {
      matchedSubscriptionIds.add(mapped.subscriptionId);
      rows.push({
        mms_id: student.mmsId,
        student_name: student.fullName || '',
        payer_label: payer.payerLabel,
...
export function buildStripeAmountsMap(cacheRows = [], { now = new Date(), maxAgeDays = 14 } = {}) {
  ...
  let staleCount = 0;
  // A split household contributes one row per subscription, and
  // resolveStudentRevenue reads this value as the student's whole weekly
  // revenue — so rows are summed, not overwritten. Deduplicated by subscription
  // id, because summing a row that somehow appeared twice would invent money.
  ...
  for (const row of cacheRows) {
    ...
    const subscriptionId = `${row.stripe_subscription_id ?? row.stripeSubscriptionId ?? ''}`.trim();
    if (subscriptionId) {
      const seen = seenSubscriptions.get(mmsId) || new Set();
      if (seen.has(subscriptionId)) continue;
      seen.add(subscriptionId);
      seenSubscriptions.set(mmsId, seen);
    }
    const current = map[mmsId];
    const rowMonthly = Number.isFinite(monthly) ? monthly : round(weekly * WEEKS_PER_MONTH);
    map[mmsId] = current
      ? { weekly: round(current.weekly + weekly), monthly: round(current.monthly + rowMonthly) }
      : { weekly, monthly: rowMonthly };
  }
...
export function buildStripeCollectionMatchStudents(students = [], archivedStudents = [], splitBillingRows = []) {
  ...
  // One entry per (student, payer). The invoice matcher keys its lookups by
  // subscription and customer id, so a split household's second payer needs its
  // own entry pointing back at the same student. Without it that parent's paid
  // invoices land in `unmatched_collection` every month forever — a permanent
  // false positive in the one signal built to catch real billing mistakes.
  const splitBilling = buildSplitBillingIndex(splitBillingRows);
  const entries = [];
  for (const student of byMmsId.values()) {
    entries.push(student);
    for (const payer of splitBilling.get(student.mmsId) || []) {
      if (payer.subscriptionId && payer.subscriptionId === student.stripeSubscriptionId) continue;
      if (!payer.subscriptionId && payer.customerId === student.stripeCustomerId) continue;
      entries.push({
        ...student,
        stripeSubscriptionId: payer.subscriptionId,
...
export function summariseCollectedInvoices(invoices = [], { month = '', students = [], archivedStudents = [], splitBillingRows = [] } = {}) {
  ...
  const bySubscriptionId = new Map();
  const byCustomerId = new Map();
  const matchStudents = buildStripeCollectionMatchStudents(students, archivedStudents, splitBillingRows);
  for (const student of matchStudents) {
    ...
    if (customerId) {
      ...
      // "This customer identifies one student" means one student, not one entry:
      // a split household contributes several entries per student and two of
      // them can share a customer id. Counting entries would make an
      // unambiguous customer look ambiguous and drop the money to unmatched.
      if (!matches.some((existing) => existing.mmsId === student.mmsId)) {
