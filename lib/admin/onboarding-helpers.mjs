/** @fileoverview Pure onboarding step creation, duplicate detection, recovery guidance, instrument/tutor pairing, and completion assessment. */
import { normaliseInstrument, normaliseTeachingInstrument } from './fc-helpers.mjs';

// Electric and acoustic guitar share one teaching lane, so compare on the
// teaching instrument rather than the label the student is recorded under.
export function tutorTeachesInstrument(tutor, instrument) {
  const needle = normaliseTeachingInstrument(instrument).toLowerCase().trim();
  return !needle || (tutor?.instruments || []).includes(needle);
}

// The instrument decides which tutors onboarding offers, so getting it wrong
// hides the very tutor the waiting list suggested. Trust the instrument the
// suggestion was made for, then the sign-up note read against that tutor, and
// only then the first instrument the note asks for.
export function resolveOnboardingInstrument({
  requestedInstrument = '',
  noteInstruments = [],
  requestedTutor = null,
} = {}) {
  if (requestedInstrument) return normaliseInstrument(requestedInstrument);
  if (requestedTutor) {
    const taught = noteInstruments.find((instrument) => tutorTeachesInstrument(requestedTutor, instrument));
    if (taught) return taught;
  }
  return noteInstruments[0] || '';
}
...
export function buildOnboardingCompletionStatus({ steps }) {
  ...
  return {
    ...
    fcIdentityRefresh: {
      ...
      detail: 'Refreshes automatically: the first-chord-brain hourly job reads the registry on main within the hour. A local generate_fc_ids.py run reads your local checkout, so only run it after git pull.',
    },
    portalActivation: {
      status: 'pending',
...
// What must be true before "Complete onboarding" is allowed to run. Kept here
// rather than inline in the form so the rule is testable and can be reused by
// the route if the API ever needs the same guard.
//
export function findOnboardingCompletionBlockers(form = {}) {
  const blockers = [];

  // The FC student ID is minted from the MMS student ID, so without one there is
  // nothing to mint from. Fail here, before any work, rather than mid-route.
  if (!/^sdt_[A-Za-z0-9]+$/u.test(`${form?.mmsId || ''}`.trim())) {
    blockers.push({
      field: 'mmsId',
      message: 'Choose the student from the MMS waiting list first. Onboarding needs their MMS student ID.',
    });
  }

  if (!`${form?.soundsliceUrl || ''}`.trim()) {
    blockers.push({
      field: 'soundsliceUrl',
      message: 'Add the Soundslice URL before completing onboarding. The student has no practice link without it.',
    });
  }

  if (form?.humanChecks?.paymentTermsExplained !== true) {
    blockers.push({
      field: 'paymentTermsExplained',
      message: 'Confirm that the weekly Stripe subscription and first-lesson payment were explained on the welcome call.',
    });
  }

  if (form?.humanChecks?.lessonWhatsappGroupReady !== true) {
    blockers.push({
      field: 'lessonWhatsappGroupReady',
      message: 'Confirm that the lesson WhatsApp group contains the parent/student, assigned tutor, Finn, Tom and Fennella.',
    });
  }
