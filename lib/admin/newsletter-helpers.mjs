...
export function buildNewsletterItemRow({
  ...
  // An uploaded file is material as much as words are. Counting only text meant a
  // photo-only contribution lost its captured_at the next time its row was
  // rewritten — reading as "nothing yet" while the photo sat in Drive, and making
  // a second photo look like a first arrival.
  const hasMaterial = Boolean(resolvedText) || Boolean(clean(existingRow?.mediaJson));
  ...
  return {
    row: {
      ...
      updatedAt: timestamp,
      mediaJson: clean(existingRow?.mediaJson),
    },
  };
}
...
export function buildEditorialUpdate({
  ...
  if (normalised === 'selected') {
    const gate = canSelectItem(existingRow, { standingConsent, media });
...
export function isEmptyRequestRow(item = {}) {
  return Boolean(clean(item.requestedAt))
    ...
    // A row holding an uploaded file is never "untouched": deleting it would
    // orphan bytes in Drive that nothing then references.
    && !clean(item.mediaJson);
}

// True when a write moved an item from "nothing yet" to "arrived". An edit, a
// retried save with the same ticket, a second photo on an item that already
// arrived, and a "nothing this month" reply are never first arrivals — which is
// what keeps Fenella's notification to one email per item.
export function isFirstArrival({ before = null, after = null } = {}) {
  return !clean(before?.capturedAt) && Boolean(clean(after?.capturedAt));
}
...
// An uploaded file counts as media whether or not anybody ticked the box: the
// upload route sets has_media, but deriving it from the attachments too means a
// row can never hold a child's photo while reading as "no permission needed".
export function itemHasMedia(item = {}, media = null) {
  const attachments = Array.isArray(media) ? media.length : 0;
  return isTruthy(item.hasMedia) || attachments > 0;
}

export function deriveItemConsent(item = {}, { standingConsent = null, media = null } = {}) {
  if (!itemHasMedia(item, media)) {
...
export function canSelectItem(item = {}, { standingConsent = null, media = null } = {}) {
  const consent = deriveItemConsent(item, { standingConsent, media });
...
export function summariseIssue(items = [], { issue = null, currentDate = new Date() } = {}) {
  ...
  const decorated = scoped.map((item) => {
    ...
    return {
      ...
      media,
      mediaSummary: summariseItemMedia(media),
      consent: deriveItemConsent(item, { standingConsent, media }),
