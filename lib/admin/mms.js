...
import { parseInstrumentList } from './fc-helpers.mjs';
...
import { deriveStudentAge } from './student-age.mjs';
import {
...
} from './practice-notes-group-helpers.mjs';
import {
...
function parseWaitingInstruments(note = '') {
  ...
  return {
    rawInstrument,
    // Shared with onboarding so both surfaces read the same instruments out of
    // the same note.
    instruments: parseInstrumentList(rawInstrument),
    parsed,
  };
}
...
// Ages move once a year, so one all-students read serves every render for hours.
// MMS stays the only home for the fact: an exact DateOfBirth typed into MMS
// wins over the sign-up note on the next refresh.
const studentAgeCache = createSwrCache({
  ttlMs: 6 * 60 * 60 * 1000,
  staleWhileRevalidateMs: 18 * 60 * 60 * 1000,
  label: 'MMS student ages',
});

async function fetchStudentAges() {
  const rows = await fetchAllPages(async ({ offset, limit }) => {
    const response = await mmsFetch(`${MMS_BASE_URL}/search/students?offset=${offset}&limit=${limit}`, {
      method: 'POST',
      headers: getMmsHeaders(),
      body: JSON.stringify({
        IDs: [],
    ...
    if (!response.ok) {
      ...
      throw new Error(`MMS student age search failed: ${response.status}${details ? ` - ${details}` : ''}`);
    }
    return { rows: responseBody?.ItemSubset || [], total: responseBody?.TotalItemCount };
  }, { pageSize: 1000, label: 'MMS student ages' });

  const ages = {};
  for (const row of rows) {
    const age = deriveStudentAge({ dateOfBirth: row.DateOfBirth, note: row.Note, dateStarted: row.DateStarted });
    if (row.ID && age) ages[row.ID] = age;
  }
  return ages;
}

// mmsId → derived age. Never throws: age is garnish on a record page, so an MMS
// outage drops the chip rather than the page.
export async function getStudentAgesByMmsId() {
  try {
    return await studentAgeCache.read('all', fetchStudentAges, { staleOnError: true });
  } catch (error) {
    console.warn('MMS student ages unavailable:', error.message);
    return {};
  }
}
...
// inclusive of periodEnd (a Monday run covers through the Sunday), so shift
...
export async function getValidatedMmsFreeCalendarSlot({
  ...
  // The pinned event only proves the slot was free in its own week. When the
  // first lesson is bumped forward, confirm the slot is still free in the week
  // it will actually start, so a bump can never book over something else.
  if (!slot.weekOffset) {
    return slot;
  }

  const dayEvents = await searchCalendarEventsInRange({
    teacherId,
    startDate: lessonDate,
    endDate: addDaysToCalendarDate(lessonDate, 1),
  });

  const occurrence = findFreeCalendarOccurrence({
    events: dayEvents,
    teacherId,
    startDateTime: slot.lessonStartDateTime,
    durationMinutes: slot.durationMinutes,
  });

  if (!occurrence) {
    throw new Error(`The selected Free slot is not open on ${lessonDate} at ${lessonTime}. Choose a week it is still free, or onboard without the slot.`);
  }
...
// Everyone on one lesson, read from the event rather than inferred. A shared
// MMS event carries one attendance record per student, so this is the exact
// membership of a group lesson — not the Schedule_Context shared-slot guess,
// which pairs students by matching next-lesson timestamps and quietly breaks
// when one side's cache was refreshed on a different day.
export async function getMmsEventAttendances(eventId = '') {
  if (!/^evt_[A-Za-z0-9_-]+$/u.test(`${eventId || ''}`)) {
    throw new Error('A valid MMS event ID is required to read a lesson group.');
  }
  ...
  if (!response.ok) {
    const details = formatMmsErrorBody(responseBody);
    const error = new Error(`MMS lesson group read failed: ${response.status}${details ? ` - ${details}` : ''}`);
    error.status = response.status;
    throw error;
  }

  const event = unwrapMmsCalendarEvent(responseBody);
  return Array.isArray(event?.Attendances) ? event.Attendances : [];
}

// Preview for a whole shared lesson, pinned to one event.
//
// The single-student preview picks each student's own latest unrecorded record.
// Run that twice for two sisters in the same lesson and they land on different
// dates — which is exactly what happens today. Here the lead student's selected
// event fixes the lesson, and every other member is taken from that same event.
export async function previewPracticeNoteGroupWrite({
  studentId,
  noteText,
  now = new Date(),
  targetAttendanceId = '',
  ...
  const lead = await previewPracticeNoteMmsTestWrite({
    studentId,
    noteText,
    now,
    targetAttendanceId,
  ...
  const eventId = lead.targetAttendance?.eventId || '';
  const attendances = eventId ? await getMmsEventAttendances(eventId) : [];
  const group = buildPracticeNoteLessonGroup({ attendances, leadStudentMmsId: studentId });

  if (!group.isGroup) {
    return { ...lead, group: { ...group, plan: [], emailCount: 0, attendanceCount: group.memberCount, sendEmails } };
  }

  // One MMS student read per member, for their own parent contacts. The lead's
  // recipients are already in hand, so only the others cost a request.
  const others = group.members.filter((member) => !member.isLead);
  const otherRecords = await Promise.all(others.map((member) => getStudentRecord(member.studentMmsId)));
  const recipientsByStudentMmsId = new Map([
    [studentId, lead.recipients || []],
    ...others.map((member, index) => [
      member.studentMmsId,
      attendanceStatus === 'AbsentNoMakeup' ? [] : buildPracticeNoteEmailRecipients(otherRecords[index]),
    ]),
  ]);

  const delivery = planPracticeNoteGroupDelivery({
    members: group.members,
    recipientsByStudentMmsId,
    sendEmails: sendEmails && attendanceStatus !== 'AbsentNoMakeup',
  });
...
export async function previewPracticeNoteMmsTestWrite({
  ...
  // These reach a tutor mid-lesson, so they name the student in front of them.
  // They were written during the single-student pilot and hardcoded "Test
  // Studenty" — telling a tutor their real student's lesson could not be found
  // under a test account's name, which reads as a broken tool rather than as
  // missing data.
  const studentLabel = student.FullName
    || `${student.FirstName || ''} ${student.LastName || ''}`.trim()
    || studentId
    || 'this student';
  if (!targetAttendance) {
    throw new Error(targetAttendanceId
      ? `Requested MMS attendance record was not found for ${studentLabel}.`
      : `No matching MMS attendance record found for ${studentLabel}.`);
  }
  ...
  if (normalisedAttendanceStatus === 'Present' && !recipients.length) {
    throw new Error(`No email-capable MMS recipients found for ${studentLabel}.`);
  }
...
export async function executePracticeNoteMmsTestWrite({
  ...
  targetAttendanceId = '',
  attendanceStatus = 'Present',
  // Group delivery only. `emailStudentLabel` names every student the one email
  // covers ("Athena and Sophia"), and `suppressEmailReason` marks a member whose
  // household is already being emailed by a sibling — recorded as a reason, not
  // as silence, so "no email was needed" never reads like "the email failed".
  emailStudentLabel = '',
  suppressEmailReason = '',
} = {}) {
  ...
  if (suppressEmailReason) {
    practiceNoteEmail = {
      ok: true,
      skipped: true,
      channel: 'none',
      reason: suppressEmailReason,
    };
  } else if (preview.requestedAttendanceStatus === 'Present') {
    ...
    try {
      practiceNoteEmail = await sendPracticeNoteEmail({
        ...
        // Separated parents, two carers, a grandparent on the record: every
        // other MMS parent is Bcc'd on the same send rather than dropped.
        bccRecipients: preview.recipients.slice(1),
        studentName: emailStudentLabel || preview.student.name,
