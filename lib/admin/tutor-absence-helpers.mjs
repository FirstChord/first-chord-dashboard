...
// Capture cards carry opt-in modes as note lines; resolve a marker to the
// absence IDs whose capture card has it.
export function collectTutorAbsenceIdsWithMarker(planningItems = [], marker = '') {
  if (!marker) return new Set();
  return new Set(
    (planningItems || [])
      .filter((item) => `${item.notes || ''}`.includes(marker))
      .map((item) => {
        const notes = `${item.notes || ''}`;
        const tutorShortName = item.linkedTutorId || notes.match(/^Tutor:\s*(\S+)/mu)?.[1] || '';
        const absenceDate = notes.match(/^Tutor absence date:\s*(\d{4}-\d{2}-\d{2})$/mu)?.[1] || '';
        return tutorShortName && absenceDate ? buildTutorAbsenceId({ tutorShortName, absenceDate }) : '';
      })
      .filter(Boolean),
  );
}
...
export function buildTutorAbsenceCombinedMessage({
  parentName = '',
  studentName = '',
  tutorName = '',
  missedDates = [],
} = {}) {
  const dates = [...new Set((missedDates || []).filter(Boolean))].sort();
  ...
  const dateText = joinHumanList(dates.map(formatTutorAbsenceDate));
  return `Hi ${parentFirst}! Just a quick heads up that ${tutorFirst} is away on ${dateText}, so ${studentFirst}’s lesson${many ? 's' : ''} won’t be going ahead.\n\nWe’ve paused the payment for ${many ? 'those lessons' : 'that lesson'}, so there’s nothing you need to do. Thanks!`;
}

// A combined card replaces the early notice: its message is the parent's only
// one, so the card is due from the day the cancellation was recorded rather than
// a few days before the lesson. The message stays last in the notes.
function applyCombinedNotice(item = {}, { rows = [], lesson = {}, tutorName = '', missedDates = [] } = {}) {
  const decidedDates = rows
  ...
  return {
    ...
    targetDate: [item.targetDate, decidedDates[0]].filter(Boolean).sort()[0] || '',
    nextAction: item.status === 'done'
      ? item.nextAction
      : 'Run the pause tool, then send the one message that tells the parent about the absence and the pause.',
  };
}
    ...
    for (const block of splitCandidatesIntoBlocks(studentCandidates)) {
      if (block.length < 2) {
        ...
        plans.push(...buildTutorAbsencePausePlanningItems({
          ...
          combinedNotice: Boolean(candidate.row.combinedNotice),
      ...
      const combinedRows = block.map((candidate) => candidate.row).filter((row) => row.combinedNotice);

      plans.push({
        planningId,
        item: combinedRows.length
          ? applyCombinedNotice(periodItem, {
            rows: combinedRows,
            lesson: firstCandidate.lesson,
            tutorName,
            missedDates,
          })
          : periodItem,
...
export function buildTutorAbsencePausePlanningItems({
  ...
  return (lessons || []).flatMap((lesson) => {
    ...
    return [{
      planningId,
      item: combinedNotice
        ? applyCombinedNotice(lessonItem, {
          rows: [{ createdAt }],
          lesson,
          tutorName: tutorName || tutorShortName,
          missedDates: [lessonDate],
        })
        : lessonItem,
...
export function summariseTutorAbsenceState({
  ...
  const paymentHandledCount = normalisedDecision === 'cancel_day'
    ...
    : 0;
  const coverTutorSelected = Boolean(coverTutorName);
  const coverTutorConfirmed = Boolean(workflowState.coverTutorConfirmed);
  const coverTutorBriefed = Boolean(workflowState.coverTutorBriefed);
  const calendarAligned = Boolean(workflowState.calendarUpdated);
  const coverReady = normalisedDecision === 'cover'
    ? coverTutorSelected && coverTutorConfirmed && coverTutorBriefed && calendarAligned
    : false;
  ...
  let resolutionHint = '';
  if (!normalisedDecision) resolutionHint = 'Choose cancel or cover before resolving.';
  else if (normalisedDecision === 'cover' && !coverTutorSelected) resolutionHint = 'Choose a cover tutor before resolving.';
  else if (normalisedDecision === 'cover' && !coverTutorConfirmed) resolutionHint = 'Confirm the cover tutor before resolving.';
  else if (normalisedDecision === 'cover' && !coverTutorBriefed) resolutionHint = 'Pass on the lesson notes and context before resolving.';
  else if (normalisedDecision === 'cover' && !calendarAligned) resolutionHint = 'Confirm the MMS/calendar update, or that no change is needed, before resolving.';
  else if (!allMessaged) resolutionHint = 'Resolve unlocks once all affected parents are marked messaged.';
  else if (normalisedDecision === 'cancel_day' && !paymentComplete) resolutionHint = 'Resolve unlocks once every affected lesson has payment handled or explicitly skipped.';
...
export function assertTutorAbsenceResolutionReady({
  status = '',
  lessons = [],
  messageState = {},
  decision = '',
  ...
  const summary = summariseTutorAbsenceState({ lessons, messageState, decision, coverTutorName });
  if (normaliseTutorAbsenceStatus(status) !== 'resolved' || lessons.length === 0 || summary.canResolve) {
    return summary;
  }

  const error = new Error(summary.resolutionHint || 'This tutor absence is not ready to resolve.');
  error.code = 'TUTOR_ABSENCE_NOT_READY';
  throw error;
}
