...
// Does duration set this student's price at all? Group students pay a flat £20 a
// week and orchestra members a flat monthly subscription, so no disagreement
// about their duration can move money — and a card with no possible consequence
// is noise, the same reason finance coverage refuses to card its confidence-only
// flags. Asked of the pricing model with two durations rather than hardcoding
// the lesson kinds, so if a flat price ever becomes duration-banded this follows
// without being edited.
function durationSetsPrice(student) {
  return weeklyValueForDuration(student, 30).baselineWeeklyValue
    !== weeklyValueForDuration(student, 60).baselineWeeklyValue;
}
...
export function buildLessonDurationIssues(students = [], registryByMmsId = new Map()) {
  return students
    ...
    .flatMap((student) => {
      ...
      // Flat-priced students (group, orchestra) can hold every duration wrong
      // without a penny moving. Manual payers are deliberately not excluded
      // here: they are absent from the Stripe forecast but still counted in the
      // revenue estimate, so their band is real money.
      if (!durationSetsPrice(student)) return [];
      ...
      if (billingProfileDisagrees) {
        // Not "MMS would invoice wrong" — the school bills through Stripe and
        // MMS's BillingRate is never read. The real consequence is latent:
        // buildScheduleContext falls back to the billing profile duration
        // whenever a student has no upcoming calendar event, so a stale profile
        // becomes the price input the moment the calendar has a gap.
        details.push('The MMS billing profile says a different length from the booked lesson. The forecast uses the calendar while a lesson is booked and falls back to the billing profile when none is, so this misprices as soon as the calendar has a gap.');
      }
