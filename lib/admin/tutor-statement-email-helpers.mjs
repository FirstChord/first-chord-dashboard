...
export function buildTutorStatementEmailContent({
  ...
  const subject = `Your First Chord pay statement · ${period}`;
  const plainText = [
    `Hi ${firstName},`,
    '',
    `Your First Chord pay statement for ${period} is ready.`,
    '',
  ...
  const html = [
    ...
    `<p>Your First Chord pay statement for <strong>${escapeHtml(period)}</strong> is ready.</p>`,
