...
import {
  batchUpsertIncomingMessageInboxRows,
...
} from './bridge-coverage-helpers.mjs';
...
export async function recordBridgeStatus(payload = {}) {
  const bridgeId = `${payload.bridgeId || payload.bridge_id || 'primary'}`.trim() || 'primary';
  const now = new Date();

  // The live health check answers "is the bridge up?", which by the next
  // morning is always yes — the bridge restarts itself, so an evening outage
  // leaves the indicator green and no trace that anything was missed. The gap
  // between the previous heartbeat and this one is the only evidence that
  // window ever existed, so it is written down before it is overwritten.
  const previous = await getBridgeStatusRows()
    .then((rows) => rows.find((row) => row.bridgeId === bridgeId) || null)
    .catch(() => null);
  const gap = detectBridgeCoverageGap({
    previousHeartbeatAt: previous?.lastHeartbeatAt || '',
    now,
  });
  const coverageGaps = appendBridgeCoverageGap(
    parseBridgeCoverageGaps(previous?.rawJson || ''),
    gap,
  );
  ...
  const row = {
    ...
    rawJson: serialiseBridgeCoverageGaps(coverageGaps),
  };
  await upsertBridgeStatusRow(row);
  return { ...row, coverageGap: gap };
}

// The windows in the last few days when the inbox was not capturing, for the
// banner on the incoming-messages page.
export async function getBridgeCoverageGaps({ bridgeId = 'primary', withinHours = 72 } = {}) {
  const status = await getBridgeStatus({ bridgeId });
  return selectRecentBridgeCoverageGaps(parseBridgeCoverageGaps(status?.rawJson || ''), { withinHours });
}
...
function scopeIncomingMessageRows(rows, statusScope) {
  return statusScope === 'active'
    ? rows.filter((row) => ['inbox', 'needs_review'].includes(row.status || 'inbox'))
    : statusScope === 'resolved'
      ? rows.filter((row) => ['converted', 'ignored'].includes(row.status))
      : rows;
}

async function hydrateIncomingMessageRows(rows, { includePlanning = true } = {}) {
  // Active inbox rows cannot have a completed linked plan. Skipping the large
  // Planning_Items join makes the everyday queue materially faster; reporting
  // and insights can opt into it when they actually use linked-plan outcomes.
  const needsPlanningJoin = includePlanning && rows.some((row) => row.createdPlanningId);
  const planningRows = needsPlanningJoin
    ? await getPlanningItemRows().catch((error) => {
...
export async function getIncomingMessageInbox({ statusScope = 'all' } = {}) {
  const rows = await getIncomingMessageInboxRows();
  return hydrateIncomingMessageRows(groupIncomingMessages(scopeIncomingMessageRows(rows, statusScope)));
}

export async function getIncomingMessageInboxPage({ statusScope = 'active', limit = 100, includePlanning = false } = {}) {
  const rows = await getIncomingMessageInboxRows();
  const scopedRows = groupIncomingMessages(scopeIncomingMessageRows(rows, statusScope));
  const visibleRows = limit > 0 ? scopedRows.slice(0, limit) : scopedRows;
  const lastAutoCaptureAt = groupIncomingMessages(rows)
    .find((row) => row.source === AUTO_CAPTURE_SOURCE)?.capturedAt || '';
  return {
    inbox: await hydrateIncomingMessageRows(visibleRows, { includePlanning }),
    totalCount: scopedRows.length,
    autoArchivedCount: scopedRows.filter(isAutoArchivedMessage).length,
    lastAutoCaptureAt,
    limit: limit > 0 ? limit : null,
  };
}

export async function getIncomingMessageConversationContext({ incomingId = '', limit = 4 } = {}) {
  const rows = await getIncomingMessageInboxRows();
  return selectIncomingConversationContext(rows, incomingId, { limit }).map((row) => ({
...
export async function updateIncomingMessageReview({ incomingId, status = '', reviewNote = '', createdPlanningId = '', resolutionType = '', classificationActionability = '', actorEmail = '' }) {
  ...
  const next = applyIncomingMessageReview(row, {
    status,
    reviewNote,
    createdPlanningId,
    resolutionType,
...
export async function updateIncomingMessageReviews({
  incomingIds = [],
  status = '',
  reviewNote = '',
  resolutionType = '',
  ...
  if (!ids.length) throw new Error('At least one incomingId is required');

  const existing = await getIncomingMessageInboxRows();
  const byId = new Map(existing.map((row) => [row.incomingId, row]));
  const missingId = ids.find((id) => !byId.has(id));
  if (missingId) throw new Error(`Incoming message ${missingId} was not found`);
  const now = new Date();
  const nextRows = ids.map((incomingId) => applyIncomingMessageReview(byId.get(incomingId), {
    status,
    reviewNote,
    resolutionType,
    classificationActionability,
  ...
  await batchUpsertIncomingMessageInboxRows(nextRows);
  return nextRows;
}

export async function restoreIncomingMessageReviews({ snapshots = [], actorEmail = '' } = {}) {
  const usableSnapshots = snapshots
    .filter((snapshot) => snapshot?.incomingId)
    .slice(0, 12);
  if (!usableSnapshots.length) throw new Error('At least one undo snapshot is required');

  const existing = await getIncomingMessageInboxRows();
  const byId = new Map(existing.map((row) => [row.incomingId, row]));
  const now = new Date();
  const nextRows = usableSnapshots.map((snapshot) => {
    const row = byId.get(`${snapshot.incomingId || ''}`.trim());
    if (!row) throw new Error(`Incoming message ${snapshot.incomingId} was not found`);
    if (!snapshot.expectedReviewedAt || row.reviewedAt !== snapshot.expectedReviewedAt) {
      const error = new Error('This message changed after that action, so Undo was not applied. Refresh and review the latest state.');
      error.status = 409;
      throw error;
    }
    return restoreIncomingMessageWorkflowState(row, snapshot, { actorEmail, now });
  });
...
export async function snoozeIncomingMessages({ incomingIds = [], snoozedUntil = '', actorEmail = '' } = {}) {
  const ids = [...new Set(incomingIds.map((id) => `${id || ''}`.trim()).filter(Boolean))];
  if (!ids.length) throw new Error('At least one incomingId is required');
  ...
  if (missingId) throw new Error(`Incoming message ${missingId} was not found`);
  const now = new Date();
  const nextRows = ids.map((incomingId) => applyIncomingMessageSnooze(byId.get(incomingId), {
    snoozedUntil,
    actorEmail,
    now,
  }));
