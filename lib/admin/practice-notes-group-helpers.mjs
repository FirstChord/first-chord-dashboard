...
export function planPracticeNoteGroupDelivery({
  ...
  const plan = members.map((member) => {
    ...
    if (!carriers.has(key)) {
      ...
      // Only the carrier sends, so only the carrier carries the Bcc'd parents.
      // Deduplication above is by the primary address on purpose: siblings in
      // one household still get one email, and that one email copies the
      // household's other parents.
      return {
        ...member,
        recipient,
        bccRecipients: recipients.slice(1),
        emailRole: 'carrier',
        emailReason: '',
        covers: [member.studentName],
      };
    }
    return {
      ...member,
