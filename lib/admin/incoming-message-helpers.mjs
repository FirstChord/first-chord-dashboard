/** @fileoverview Pure core of the incoming-message inbox: classification, student and tutor matching, snoozing, WhatsApp group sync, reply templates, and planning drafts. */
import { matchWhatsappTutorGroup } from './incoming-tutor-group-helpers.mjs';
...
export function matchIncomingMessageToStudent(message = {}, students = [], { groupMapRows = [] } = {}) {
  const tutorGroup = groupMapRows.find(row => row.chatId === message.chatId
    && row.status === 'confirmed' && row.groupType === 'tutor' && row.matchedTutorId);
  if (tutorGroup || message.groupType === 'tutor') {
    return { matchedMmsId: '', matchedStudentName: '', matchConfidence: 'none',
      matchReasons: 'tutor group — select a student explicitly if this message concerns one', score: 0 };
  }
...
export function buildIncomingMessageRecord(payload = {}, { students = [], groupMapRows = [], now = new Date() } = {}) {
  ...
  return {
    ...message,
    groupType: tutorGroup ? 'tutor' : 'student',
    matchedTutorId: tutorGroup?.matchedTutorId || '',
    matchedTutorName: tutorGroup?.tutorName || '',
...
// Apply the human outcome to one stored row. Keeping this transformation pure
// lets the server write an entire WhatsApp burst in one batch while preserving
// the same classification/audit semantics as the original one-row endpoint.
export function applyIncomingMessageReview(row = {}, {
  status = '',
  reviewNote = '',
  createdPlanningId = '',
  resolutionType = '',
...
// A short-lived client Undo carries only the workflow fields that existed
// before the press. The server still reloads the current row and refuses to
// restore over a newer review, so this snapshot is never treated as truth by
// itself.
export function buildIncomingUndoSnapshot(row = {}) {
  return {
    incomingId: clean(row.incomingId),
    status: normaliseEnum(row.status, ['inbox', 'needs_review'], 'inbox'),
    snoozedUntil: clean(row.snoozedUntil),
    classificationActionability: normaliseEnum(
      row.classificationActionability,
      INCOMING_MESSAGE_ACTIONABILITY,
      'uncertain',
    ),
    classificationDecision: normaliseEnum(
      row.classificationDecision,
      ['unreviewed', 'accepted', 'corrected', 'legacy'],
      'legacy',
    ),
...
export function restoreIncomingMessageWorkflowState(row = {}, snapshot = {}, {
  actorEmail = '',
  now = new Date(),
} = {}) {
  if (!clean(snapshot.incomingId) || clean(snapshot.incomingId) !== clean(row.incomingId)) {
    throw new Error('Undo snapshot does not match the incoming message');
  }
  if (clean(row.createdPlanningId)) {
    throw new Error('A message linked to Planning cannot be undone from the inbox');
  }

  const status = normaliseEnum(snapshot.status, ['inbox', 'needs_review'], '');
  if (!status) throw new Error('Undo can restore only an open incoming message');

  const requestedSnooze = clean(snapshot.snoozedUntil);
  const snoozeMs = new Date(requestedSnooze).getTime();
  const snoozedUntil = requestedSnooze && Number.isFinite(snoozeMs) && snoozeMs > now.getTime()
    ? new Date(snoozeMs).toISOString()
    : '';
  ...
  return {
    ...
    snoozedUntil,
    classificationActionability: normaliseEnum(
      snapshot.classificationActionability,
      INCOMING_MESSAGE_ACTIONABILITY,
      row.classificationActionability || 'uncertain',
    ),
    classificationDecision: normaliseEnum(
      snapshot.classificationDecision,
      ['unreviewed', 'accepted', 'corrected', 'legacy'],
      row.classificationDecision || 'legacy',
    ),
    reviewNote: clean(snapshot.reviewNote),
    reviewedBy: actorEmail || row.reviewedBy || '',
    reviewedAt: now.toISOString(),
  };
}

// Merge the compact response from an inbox mutation into the client copy. The
// server remains authoritative for the changed rows; untouched rows and their
// scroll position stay in place instead of being replaced by a full reload.
export function mergeIncomingInboxMutation(rows = [], {
  updatedMessages = [],
  deletedIncomingIds = [],
  now = new Date(),
} = {}) {
  const updates = new Map(updatedMessages
    .filter((row) => row?.incomingId)
    .map((row) => [row.incomingId, row]));
  const deleted = new Set(deletedIncomingIds.filter(Boolean));
  const seen = new Set();
  const next = rows
    .filter((row) => !deleted.has(row.incomingId))
    .map((row) => {
      const update = updates.get(row.incomingId);
      if (!update) return row;
      seen.add(row.incomingId);
      const merged = { ...row, ...update };
      return { ...merged, isSnoozed: isIncomingMessageSnoozed(merged, { now }) };
    });

  for (const [incomingId, update] of updates) {
    if (seen.has(incomingId) || deleted.has(incomingId)) continue;
    next.push({ ...update, isSnoozed: isIncomingMessageSnoozed(update, { now }) });
  }
  return groupIncomingMessages(next);
}

export function selectAdjacentIncomingId(clusters = [], currentIncomingId = '') {
  const index = clusters.findIndex((cluster) => cluster?.lead?.incomingId === currentIncomingId);
  if (index === -1) return clusters[0]?.lead?.incomingId || '';
  return clusters[index + 1]?.lead?.incomingId
    || clusters[index - 1]?.lead?.incomingId
    || '';
}

export function retainIncomingSelection(clusters = [], selectedIncomingId = '') {
  return clusters.some((cluster) => cluster?.lead?.incomingId === selectedIncomingId)
    ? selectedIncomingId
    : clusters[0]?.lead?.incomingId || '';
}
...
export function buildWhatsappGroupMapRecord(record = {}, existing = {}) {
  ...
  const groupType = record.groupType || existing.groupType || 'student';
  const isTutor = groupType === 'tutor';
  return {
    groupType,
    matchedTutorId: isTutor ? record.matchedTutorId ?? existing.matchedTutorId ?? '' : '',
    ...
    matchedMmsId: isTutor ? '' : record.matchedMmsId || existing.matchedMmsId || '',
    matchedFcId: isTutor ? '' : record.matchedFcId || existing.matchedFcId || '',
    matchedStudentName: isTutor ? '' : record.matchedStudentName || existing.matchedStudentName || '',
    additionalMmsIds: isTutor ? '' : record.additionalMmsIds ?? existing.additionalMmsIds ?? '',
    parentName: isTutor ? '' : record.parentName || existing.parentName || '',
    parentPhone: isTutor ? '' : record.parentPhone || existing.parentPhone || '',
    tutorName: isTutor
      ? record.matchedTutorName ?? record.tutorName ?? existing.tutorName ?? ''
      : record.tutorName ?? existing.tutorName ?? '',
...
// Student-group discovery uses an instrument token; tutor groups have their
// own roster-checked title convention. The
...
// importing: title has an instrument or matches a roster tutor + First Chord,
// and the group must have been
...
export function buildGroupSyncPlan({ groups = [], students = [], tutors = [], now = new Date(), activeWithinMonths = 6 } = {}) {
  ...
  for (const group of groups) {
    ...
    const tutorMatch = matchWhatsappTutorGroup(chatName, tutors);
    const instrument = detectInstrumentInName(chatName, rosterInstruments);
    if (!instrument && !tutorMatch) {
    ...
    const match = tutorMatch
      ? { ...tutorMatch, matchedMmsId: '', matchedStudentName: '' }
      : matchGroupToStudent({ chatName, participantPhones: group.participantPhones || [] }, students);
    if (match.matchedMmsId || match.matchedTutorId) summary.matched += 1;
    summary.kept += 1;

    records.push({
      chatId,
      chatName,
      groupType: tutorMatch ? 'tutor' : 'student',
      matchedTutorId: tutorMatch?.matchedTutorId || '',
      tutorName: tutorMatch?.tutorName || '',
      instrument,
      lastActiveAt,
      matchedMmsId: match.matchedMmsId,
      matchedStudentName: match.matchedStudentName,
...
export function buildIncomingReplyTemplate({
  ...
  if (groupType === 'tutor') {
    return 'Thanks for letting us know — I’ll take a look and come back to you shortly.';
  }
...
export function buildIncomingPlanningDraft({ record = {}, student = {}, replyTemplate = '', extraction = null, now = new Date() } = {}) {
  ...
  if (record.groupType === 'tutor') {
    return {
      title: `Tutor message: ${record.matchedTutorName || record.senderName || 'Tutor'}`,
      notes: [datesLine, ...contextLines].filter(Boolean).join('\n'),
      itemType: 'action', owner: 'Unassigned', status: 'inbox', area: 'tutor',
      isPause: false, linkedTutorId: record.matchedTutorId || '',
      linkedStudentIds: record.matchedMmsId ? [record.matchedMmsId] : [],
      nextAction: 'Review the tutor message and agree the next step.', targetDate: '',
    };
  }
...
// A parent firing off three messages in a row is one thing to deal with, not
// three. The bridge posts one row per WhatsApp message and that stays the
// canonical record — the burst is reassembled here, for display and for
// applying one outcome to the whole run.
export const INCOMING_BURST_WINDOW_MS = 5 * 60 * 1000;

function burstTimeMs(entry = {}) {
  const ms = new Date(clean(entry.messageAt) || clean(entry.capturedAt) || '').getTime();
  return Number.isFinite(ms) ? ms : NaN;
}

// Same conversation and same person. A shared sibling group can carry messages
// about different children, so a differing student match splits the burst
// rather than merging two children's business into one card.
function burstIdentity(entry = {}) {
  const chat = clean(entry.chatId) || phoneComparable(entry.senderPhone) || normaliseText(entry.chatName);
  const sender = phoneComparable(entry.senderPhone) || normaliseText(entry.senderName);
  if (!chat || !sender) return '';
  return `${chat}::${sender}::${clean(entry.matchedMmsId)}::${entry.groupType || 'student'}::${clean(entry.matchedTutorId)}`;
}

const BURST_LEAD_RANK = {
  action_needed: 3,
  reply_needed: 2,
  uncertain: 1,
  no_action: 0,
...
// The message the burst is actually about: "Hi Finn" / "Amy can't make
// Thursday" / "thanks!" should be planned and replied to from the middle one.
function selectBurstLead(entries = []) {
  return entries.reduce((best, entry) => {
    if (!best) return entry;
    const score = (candidate) => [
      isIncomingPlaceholderText(candidate.messageText) ? 0 : 1,
      BURST_LEAD_RANK[candidate.classificationActionability] ?? 1,
      collapse(candidate.messageText).length,
    ];
    const [bestPlaceholder, bestRank, bestLength] = score(best);
    const [entryPlaceholder, entryRank, entryLength] = score(entry);
    if (entryPlaceholder !== bestPlaceholder) return entryPlaceholder > bestPlaceholder ? entry : best;
    if (entryRank !== bestRank) return entryRank > bestRank ? entry : best;
    if (entryLength !== bestLength) return entryLength > bestLength ? entry : best;
    return best;
  }, null);
}

// Takes the already-filtered, newest-first list the view is about to render and
// merges consecutive runs from one sender. Only adjacent entries merge, so a
// burst interrupted by someone else's message stays two cards, and the list
// keeps its existing order. Entries inside a burst come back oldest-first —
// that is the order they were written in.
export function clusterIncomingMessages(entries = [], { windowMs = INCOMING_BURST_WINDOW_MS } = {}) {
  const clusters = [];

  for (const entry of entries) {
    const identity = burstIdentity(entry);
    const previous = clusters[clusters.length - 1];
    const previousEntry = previous?.entries[0];
    const gap = previous && identity
      ? Math.abs(burstTimeMs(previousEntry) - burstTimeMs(entry))
      : NaN;

    if (previous && identity && previous.identity === identity && Number.isFinite(gap) && gap <= windowMs) {
      // The incoming list is newest-first, so each later arrival is older.
      previous.entries.unshift(entry);
      continue;
    }
  ...
  return clusters.map(({ entries: burst }) => {
    const lead = selectBurstLead(burst) || burst[0];
    return {
      clusterId: burst.map((entry) => entry.incomingId).join('+'),
      lead,
      entries: burst,
    };
  });
...
function incomingConversationIdentity(entry = {}) {
  const chatId = clean(entry.chatId);
  if (chatId) return `chat:${chatId}`;
  const studentId = clean(entry.matchedMmsId);
  if (studentId) return `student:${studentId}`;
  const senderPhone = phoneComparable(entry.senderPhone);
  if (senderPhone) return `phone:${senderPhone}`;
  const senderName = normaliseText(entry.senderName);
  return senderName ? `sender:${senderName}` : '';
}

// Recent context is selected from the already-cached inbox tab only when one
// card is opened. It deliberately excludes the current five-minute burst and
// never joins Planning, students, the group map, or Communication_Log.
export function selectIncomingConversationContext(rows = [], incomingId = '', { limit = 4 } = {}) {
  const ordered = groupIncomingMessages(rows);
  const target = ordered.find((row) => row.incomingId === clean(incomingId));
  const identity = incomingConversationIdentity(target);
  if (!target || !identity) return [];

  const currentCluster = clusterIncomingMessages(ordered)
    .find((cluster) => cluster.entries.some((row) => row.incomingId === target.incomingId));
  const currentIds = new Set((currentCluster?.entries || [target]).map((row) => row.incomingId));
  const currentStartMs = Math.min(...(currentCluster?.entries || [target])
    .map((row) => burstTimeMs(row))
    .filter(Number.isFinite));
  const boundedLimit = Math.max(1, Math.min(Number(limit) || 4, 6));

  return ordered
    .filter((row) => (
      !currentIds.has(row.incomingId)
      && incomingConversationIdentity(row) === identity
      && (!Number.isFinite(currentStartMs) || burstTimeMs(row) < currentStartMs)
    ))
