/** @fileoverview Builds a Wise batch-payment CSV from reviewed payroll rows and tutor recipient records. */
...
// them all. If the duplicates disagree on amount, the tutor is held out until the
// conflict is resolved; a warning alone cannot authorise either amount.
export function selectPayableReviewedRuns(savedRuns = [], { now = new Date() } = {}) {
  const byTutor = new Map();
  const disputed = [];
  const blockedTutorKeys = new Set();
  // One unresolved reviewed statement blocks every other reviewed row for that
  // tutor. This prevents a newer normal row bypassing an earlier query or
  // awaiting-confirmation statement.
  for (const raw of savedRuns) {
    ...
    if (key && !isPayrollRunReadyForPayment(raw, { now })) blockedTutorKeys.add(key);
  }
  for (const raw of savedRuns) {
    const tutor = clean(raw.tutor ?? raw.Tutor);
    const tutorShortName = clean(raw.tutor_short_name ?? raw.tutorShortName);
    const key = norm(tutorShortName || tutor);
    if (!isPayrollRunReadyForPayment(raw, { now })) {
      if (norm(raw.status ?? raw.Status) === 'reviewed' && norm(raw.tutor_response ?? raw.tutorResponse) === 'disputed') {
        disputed.push({
    ...
    if (blockedTutorKeys.has(key)) continue;
    ...
    entry.push({
      ...
      periodStart: clean(raw.period_start ?? raw.periodStart),
  ...
  for (const entry of byTutor.values()) {
    ...
    if (distinctAmounts.length > 1) {
      ...
      continue; // Conflicting money cannot enter a payment file.
    }
    rows.push({
      status: 'reviewed',
...
export function buildWiseBatch({ rows = [], wiseByKey = new Map() } = {}) {
  ...
  for (const row of rows) {
    ...
    includedTutors.push({
      ...
      recipientName: recipient.name || row.tutor,
      tutor: row.tutor || row.tutorShortName || recipient.name,
      owedAmount: owed,
    });
    csvRows.push({
