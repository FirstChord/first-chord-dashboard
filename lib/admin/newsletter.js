    ...
    return {
      ...
      consentMessage: item.consent.required && item.consent.status !== 'cleared'
        ? buildConsentRequestMessage({
          ...
          monthLabel,
          mediaDescription: describeMediaForConsent(item.media || []),
        })
        : '',
    };
  });
...
export async function recordNewsletterContribution({
  ...
  await upsertNewsletterItemRow(result.row);
  const firstArrival = isFirstArrival({ before: existingRow, after: result.row });
  return { item: result.row, firstArrival };
}
...
export async function setNewsletterEditorial({ itemId = '', editorial = '' } = {}) {
  ...
  const result = buildEditorialUpdate({
    existingRow,
    editorial,
    standingConsent: standing,
    // Re-read from the row rather than trusting the client: the guard on a
    // child's photo must not be decidable by the page that wants past it.
    media: parseItemMedia(existingRow.mediaJson),
  });
  if (result.error) return { error: result.error };
...
// What is already attached, read before an upload so the per-item cap is checked
// against reality rather than against an empty list the client supplied.
export async function getNewsletterItemMedia({
  issueMonth = '',
  fcStudentId = '',
  itemId = '',
  captureTicket = '',
  ...
  const items = await getNewsletterItemRows(month);
  const target = `${itemId || ''}`.trim()
    || buildNewsletterItemId({ issueMonth: month, fcStudentId, captureTicket });
  const row = items.find((item) => item.itemId === target) || null;
  return { itemId: target, row, media: parseItemMedia(row?.mediaJson) };
}

// Records an already-uploaded Drive file against an item, creating the item if
// this is the first thing to arrive for that student this month.
//
// Deliberately called AFTER the bytes are in Drive. If this write then fails the
// file is an orphan, which is recoverable garbage a human can re-attach or
// delete; the reverse order would leave a row pointing at nothing, which is a
// broken page. reconcileNewsletterMedia finds those orphans.
export async function attachNewsletterMedia({
  issueMonth = '',
  mmsId = '',
  itemId = '',
  captureTicket = '',
  ...
  const students = await getOperationalAdminStudents();
  const student = students.find((candidate) => candidate.mmsId === `${mmsId || ''}`.trim());
  if (!student) return { error: 'student_not_found' };
  ...
  // Ensure the row exists and carries has_media, without disturbing text.
  const base = buildNewsletterItemRow({
    issueMonth: month,
    fcStudentId: identity.fcStudentId,
    studentName: identity.studentName,
    mmsId: identity.mmsId,
  ...
  const media = addItemMedia(parseItemMedia(existingRow?.mediaJson), entry);
  const row = { ...base.row, mediaJson: serialiseItemMedia(media) };

  // A file arriving is material arriving, even with no words attached: without
  // this a photo-only contribution would read as "nothing yet" on Fenella's
  // screen while sitting in Drive.
  if (!row.capturedAt) {
    row.capturedAt = new Date().toISOString();
    row.capturedBy = `${actorEmail || identity.tutorName || ''}`.trim();
  }

  await upsertNewsletterItemRow(row);
  // Compared against the row as it was before this upload.
  const firstArrival = isFirstArrival({ before: existingRow, after: row });
  return { item: row, media, firstArrival };
}

// Detaching a reference. The Drive file is deliberately NOT deleted: removing a
// child's photo is not something to automate, and data-protection.md does not
// authorise automated deletion. The caller is told what became unreferenced.
export async function detachNewsletterMedia({ itemId = '', driveFileId = '' } = {}) {
  const allItems = await getNewsletterItemRows();
  const existingRow = allItems.find((item) => item.itemId === `${itemId || ''}`.trim()) || null;
  if (!existingRow) return { error: 'item_not_found' };

  const { media, removed } = removeItemMedia(parseItemMedia(existingRow.mediaJson), driveFileId);
  if (!removed) return { error: 'media_not_found' };

  await upsertNewsletterItemRow({
    ...existingRow,
    mediaJson: serialiseItemMedia(media),
    updatedAt: new Date().toISOString(),
  });

  return { item: { ...existingRow, mediaJson: serialiseItemMedia(media) }, detached: removed, media };
}

// Media reconciliation (orphans vs missing files) lives in
// newsletter-media-reconcile.mjs, whose import chain is alias-free so the
// terminal report can run outside Next.
export { reconcileNewsletterMedia } from './newsletter-media-reconcile.mjs';
