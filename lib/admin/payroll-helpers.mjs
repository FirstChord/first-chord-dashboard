...
// The one MMS attendance query the payroll lane makes, derived from the pay date
...
export function buildPayrollAttendanceQuery(payDate = nextWednesday()) {
  const cleanPayDate = toInputDate(`${payDate}T12:00:00Z`);
  return {
...
export function resolveTutorPayrollWindow({
  ...
  const cleanPayDate = toInputDate(`${payDate}T12:00:00Z`);
  // Default end is the day before pay date (Tuesday before a Wednesday run). Invoices are
...
export function buildPayrollPreview({
  ...
  for (const tutor of tutors) {
    ...
    rows.push({
      ...
      paymentRoute: saved?.paymentRoute || payInfo.paymentRoute || 'normal',
