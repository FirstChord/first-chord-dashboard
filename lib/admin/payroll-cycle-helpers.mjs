...
// Approved cutoff in London local time, including DST. No timer sends or pays.
...
// Cadence is anchored to the shared cutover Monday, never to a payment date.
export function sharedPayrollDueDate({ periodStart, cadenceDays = 7, cycleDate }) {
  const earliest = shiftPayrollDate(periodStart < CONFIRMATION_SYSTEM_START ? CONFIRMATION_SYSTEM_START : periodStart, cadenceDays);
  const target = cycleDate > earliest ? cycleDate : earliest;
  const elapsed = (Date.parse(target) - Date.parse(CONFIRMATION_SYSTEM_START)) / 86400000;
  return shiftPayrollDate(CONFIRMATION_SYSTEM_START, Math.max(1, Math.ceil(elapsed / cadenceDays)) * cadenceDays);
}
