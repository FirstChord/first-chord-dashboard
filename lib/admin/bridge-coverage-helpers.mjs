...
export function describeBridgeCoverageGap(gap = null) {
  ...
  const length = hours >= 1
    ...
    : `${gap.minutes} minutes`;
  // One line. It only appears for a hole the replay could not fill, so it says
  // when, for how long, and stops — the reader can see it is an inbox.
  return `${formatWindow(gap.from)} to ${formatWindow(gap.to)} (${length}) not captured.`;
}

// Gaps worth putting in front of a human — which is far fewer than the gaps
// worth recording.
//
// The bridge replays the last 24 hours on every reconnect, and a reconnect is
// exactly what ends a gap. So a gap shorter than that window has already been
// backfilled by the time anyone reads the page, and announcing it is crying
// wolf about a hole the system has filled. Since the bridge crash-loops most
// days, showing those would make the banner permanent furniture — and a warning
// that is always on is not a warning.
//
// Only a gap longer than the replay window has a part the replay could not
// reach. Those are the ones where messages are genuinely gone.
//
// Everything is still recorded in Bridge_Status either way: this filters the
// display, not the evidence.
export function selectUnrecoveredBridgeCoverageGaps(gaps = [], {
  now = new Date(),
  withinHours = 72,
  recoverableWithinHours = 24,
} = {}) {
  const nowMs = now instanceof Date ? now.getTime() : Date.now();
  const cutoff = nowMs - withinHours * 60 * 60 * 1000;
  const recoverableMinutes = recoverableWithinHours * 60;
  return gaps
    .filter((gap) => {
      const to = toMs(gap.to);
      if (to === null || to < cutoff) return false;
      return Number(gap.minutes) > recoverableMinutes;
    })
    .sort((a, b) => `${b.to}`.localeCompare(`${a.to}`));
}

// Every recent gap, recovered or not. Kept for diagnosis rather than display.
