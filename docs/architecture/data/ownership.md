...
| Open a newsletter issue / assign priorities | authoritative | `Newsletter_Issues`, `Newsletter_Items` | admin dashboard workflow | `/admin/newsletter` | Creates the month's issue and one item row per priority student; adds one deterministic Planning Action for the deadline | Re-assigning the same student is an idempotent no-op. Removing a priority never deletes an arrival: the row is demoted to an unsolicited extra, and is removed outright only when nothing has been captured, answered or asked | Nothing is sent. The per-tutor request text is copied to WhatsApp by a human and recorded in `Communication_Log` |
| Record a newsletter contribution | authoritative | `Newsletter_Items` | admin dashboard workflow (tutor surface in a later slice) | `/admin/newsletter` | Sets `captured_at` and the contribution text against a stored `fcStudentId` | A student with no stored FC ID, or whose Sheet and registry IDs disagree, is refused (`fc_identity_unresolved` / `fc_identity_conflict`) rather than recorded under a name. FC IDs are never minted here | `captured_at` means material arrived; `nothing_this_month` is a closure and does not set it |
| Record newsletter media consent | authoritative | `Newsletter_Items.consent_*` | admin dashboard workflow | `/admin/newsletter` | Records whether a parent agreed to a picture: `no`, `yes_once`, or `yes_ongoing` | Standing consent is derived from a real `yes_ongoing` answer and is **newsletter-scoped only**; a decline is never inferred into a standing refusal, and past declines are shown as history for a human decision | The dashboard drafts the ask; a human sends it. `Event_Log` records the answer |
| Select a newsletter contribution | authoritative | `Newsletter_Items.editorial` | admin dashboard workflow | `/admin/newsletter` | Marks material as used or not used for the issue | An item with `has_media` cannot be selected until consent is cleared; the guard is re-evaluated server-side from current evidence, never trusted from the client | Selection is Fenella's editorial judgement, never a model's |
| Upload newsletter media | authoritative for bytes | **Google Drive** for the file; `Newsletter_Items.media_json` for the reference | tutor dashboard upload streamed through `POST /api/newsletter/media` | `/dashboard` (tutor), gated | Stores a Drive file ID against the student's item and sets `has_media`, which makes parental permission required before the item can be selected | Refuses entirely unless `TUTOR_DASHBOARD_AUTH_MODE` is enforced, and before reading a byte if the Drive credential is absent. Type allowlist and per-kind size caps, the cap re-checked against arriving bytes because `Content-Length` is a claim. Drive is written **before** Sheets: an orphaned file is recoverable, a reference to a missing file is not. A failed reference is reported explicitly with "do not re-upload" and found later by `npm run newsletter:media-report` | Scope is `drive.file` only — the app can touch nothing in Drive it did not create. Nothing in the code deletes a file; detaching a reference deliberately leaves the bytes in place |
...
| Lesson length | authoritative for operational workflows; **the MMS calendar wins for pricing** | `Students` sheet; `Schedule_Context` (cached MMS calendar) for the price band | manual dashboard edit or onboarding workflow | Student detail, onboarding | Operational workflows; `derivePaymentValueContext` reads `scheduleContext.durationMinutes` first and falls back to this cell | A disagreement misprices rather than fails, so it is surfaced as a `LESSON DURATION MISMATCH` issue and reconciled by a human — never auto-rewritten in either direction | Three systems hold it: MMS calendar (booked), MMS billing profile (what MMS would invoice), this cell. MMS `BillingRate` is written at onboarding and never read back |
...
| `fcStudentId` | authoritative after minting | Registry entry (the `Students` row `FC Student ID` cell is a second copy) | minted once from the MMS student ID by `generateFcStudentId(mmsId)` during onboarding or missing-registry creation; brain CLI onboarding uses the same formula (`fc_ids.make_fc_id`) | Not manually editable in dashboard | FC tabs — the brain reads the stored value and never recomputes it | Never recompute an existing value. The brain resolves registry → Students cell → `sha256(mms_id)` and raises `FC ID CONFLICT` / `FC ID MALFORMED` / `FC ID DUPLICATE` rather than overwriting | Golden value `sdt_WFQ7Js → fc_std_fa157fc5` pinned in both repos. IDs minted before 2026-09 used a name/email seed; they are still valid because the stored value wins |
...
| First Chord lesson series/event/participation ID | authoritative identity, derived facts | First Chord PostgreSQL identity; MMS for observed schedule/attendance facts | daily or operator-triggered read-only mirror sweep | Read-only `/admin/lessons` evidence, `/admin/lessons/exceptions` and `/admin/lessons/calendar`; no browser editor | Current parity reporting and fail-open shadow consumer joins | A provider-reference change creates a separate FC entity until reviewed evidence supports an alias/merge; do not heuristically merge reschedules | Calendar/exception student and tutor aliases are resolved server-side and removed before rendering; no operational workflow depends or acts on these IDs |
| Mirrored lesson schedule/attendance fields | derived provider observation | MMS | verified whole-school read, transactional SQL upsert | Daily secret-gated route or operator command; read-only parity, exception detail and weekly calendar | Parity/freshness reporting, bounded nearby-event investigation and current verified calendar only | Preserve provider ref, sync window and raw unknown values; an incomplete run writes no snapshot and disappearance never means cancellation | Rebuildable mirror, not an MMS replacement yet |
