...
| First Chord lesson series/event/participation ID | authoritative identity, derived facts | First Chord PostgreSQL identity; MMS for observed schedule/attendance facts | daily or operator-triggered read-only mirror sweep | Read-only `/admin/lessons` evidence and `/admin/lessons/calendar`; no browser editor | Current parity reporting and fail-open shadow consumer joins | A provider-reference change creates a separate FC entity until reviewed evidence supports an alias/merge; do not heuristically merge reschedules | Calendar/student/tutor aliases are resolved server-side and removed before rendering; no operational workflow depends or acts on these IDs |
| Mirrored lesson schedule/attendance fields | derived provider observation | MMS | verified whole-school read, transactional SQL upsert | Daily secret-gated route or operator command; read-only `/admin/lessons` and weekly calendar | Parity/freshness reporting, exception aggregates and current verified calendar only | Preserve provider ref, sync window and raw unknown values; an incomplete run writes no snapshot and disappearance never means cancellation | Rebuildable mirror, not an MMS replacement yet |
