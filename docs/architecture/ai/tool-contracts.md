...
| `relationship_attention.read` | Explain which teaching relationships or temporary cover episodes need attention and why now | Stable First Chord relationship/cover identities plus deterministic lifecycle, assignment, cached schedule, tutor-absence milestones, typed evidence, due date and clearing condition | A bounded explanation of the open transition or next cover step and its existing review link | Read raw Sheets/MMS rows, infer completion from an absent or stale source, turn temporary cover into a permanent assignment, change an assignment/calendar event, clear attention, or invent a tutor | Read-only. Provider IDs, contacts and practice-note text stay outside the projection. Fixtures cover open, overdue, schedule-lag and automatically cleared handovers, plus each cover milestone and resolved disappearance | Deterministic relationship schema v2, cover schema v1 and admin UI implemented; no assistant projection, route, model, or action tool |
| `lesson_occurrence.read` | Explain what is known about one dated student lesson and whether an existing follow-up remains open | Stable First Chord relationship, event and participation IDs; bounded lesson timing/tutor observation; raw attendance label; source freshness; linked tutor-absence decision; bounded practice-note delivery metadata | A concise previous/today/upcoming lesson explanation, explicit unknowns, source disagreement and an existing review link | Expose MMS/provider aliases, note text, recipients or contact data; interpret an attendance label as completion/payroll truth; infer cancellation from non-observation; change attendance, calendar, cover, assignment or workflow state | Read-only and student-scoped. Fixtures cover fresh/stale evidence, raw `Unrecorded`, exact and ambiguous note links, cover alignment/mismatch, cancellation as a separate workflow fact, and provider-ID redaction | Deterministic occurrence schema v1, bounded SQL adapter and Tutor Changes UI implemented; no assistant projection, route, model or action tool |
| `first_lesson_followup.read` | Explain what remains before one first-lesson loop can close | Exact Planning ID; redacted planned lesson and mirror evidence; recorded payment-link booleans; student-access workflow state; versioned human confirmations; deterministic blockers | A concise next-step explanation with source labels, uncertainty and links to the existing Planning/student/access screens | Expose Stripe/MMS IDs, contacts or access credentials; interpret attendance as lesson completion; confirm a step, close Planning, cancel Stripe, alter WhatsApp, or send a message | Read-only and student-scoped. Fixtures cover continue/stop, missing subscription, access workflow evidence, stale/ambiguous lesson evidence and provider-ID redaction | Deterministic read model and human Planning UI implemented; no assistant projection, route, model or action tool |
...
The Issues UI presents this as a detective case file. **Ask the detective** is
the one explicit producer-consent click: it always loads the deterministic file
and automatically requests the optional opinion when configured. The case file
may also show a **Yes** resolution selected entirely by
`getDetectiveResolution`, currently limited to source-cleared closure and the
existing primary payment-expectation correction for the two pause-expectation
types. The generated briefing supplies no action code. **No, reconsider** blocks
that proposal in the current UI and records one correction enum (`evidence_wrong`,
`diagnosis_wrong`, `proposed_fix_wrong`, `missing_context`, or `not_an_issue`)
against the opaque request ID. It does not teach the model live or change a case.
